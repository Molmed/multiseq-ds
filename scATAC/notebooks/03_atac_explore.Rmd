...
```{r}
ElbowPlot(data.filt, ndims = 30, reduction = "lsi")
```
...
data.filt <- FindClusters(object = data.filt, verbose = FALSE, algorithm = 3, resolution = 0.4)
```


```{r}
p1 = DimPlot(object = data.filt,
        shape.by = "sample",
        label = TRUE,
        pt.size = 1.5, 
        label.size = 5, 
...
Identify clostest genes to top DA peaks for each cluster. The gene list for each cluster can be used for enrichment analysis. Merge with information about the peaks. 
...
merge = merge(closest_genes, 
             GM12078.markers[,c("gene", "cluster", "p_val_adj", "avg_log2FC")], 
             by.x = "query_region", 
             by.y = "gene", 
             all.x = TRUE)
...
```
```{r}
write.csv(merge, "GM12078_DA_peaks_genes.csv")
```
